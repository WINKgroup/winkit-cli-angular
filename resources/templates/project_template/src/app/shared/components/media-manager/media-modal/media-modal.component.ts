...
export class MediaModalComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.storageService.getMediaByUserId(SessionService.getLoggedInUser()[pk]).then(s => {
  ...
  handleFileInput(files: FileList) {
    ...
    if (file.size > 3000000) {
      ...
      this.loading = true;
      this.storageService.upload(SessionService.getLoggedInUser()[pk], file, type, (progress: number) => {
